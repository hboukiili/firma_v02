...
#      with rasterio.open(path + "t2m/11.tif") as t2m, \
#             rasterio.open(path + "dt2m/11.tif") as dt2m:

#         dt2m = dt2m.read(1).astype('float64')
#         t2m = t2m.read(1).astype('float64')

#         dt2m[dt2m == 9999] = np.nan
#         t2m[t2m == 9999] = np.nan

#         Rh = 100 * (np.exp((17.625 * dt2m) / (243.04 + dt2m)) / np.exp((17.625 * t2m) / (243.04 + t2m)))
...
while current_date <= end_date_:
    ...
    et0_simple = et0_pm_simple(current_date.timetuple().tm_yday, alt, 2, 31.665358, np.nanmean(T), min(T), max(T), np.nanmean(Ws), np.nanmean(rh), min(Rh), max(Rh), irg)
