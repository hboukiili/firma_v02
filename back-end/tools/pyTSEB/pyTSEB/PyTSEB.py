...
class PyTSEB(object):
    ...
    def run(self, in_data, mask=None):
        ...
        # print("Shape of f_c:", in_data['f_c'].shape)
        # print("Shape of LAI:", in_data['LAI'].shape)
        # print("Shape of isnan(LAI):", np.isnan(in_data['LAI']).shape)
    ...
    def write_raster_output(self, outfile, output, fields):
        ...
        if driver_name in ["COG", "netCDF"]:
            ...
            for i, field in enumerate(fields):
                driver = gdal.GetDriverByName("MEM")
                out_path = join(out_dir, f"{field}.tif")
            ...
            # out_vrt = out_dir.replace('', '.vrt')
            # print(out_files)
            # gdal.BuildVRT(out_dir + '.vrt', out_files, separate=True)
